...
uint8_t ADXL345_Initialise(ADXL345 *dev, SPI_HandleTypeDef *hspi) {
	...
	/* Put sensor in full resolution mode & range +/-16g (p.27) & left justified*/
	regData = dev->range; //0x0B for right justified
...
HAL_StatusTypeDef ADXL345_ReadAcceleration(ADXL345 *dev) {
	...
	/* Convert to signed integers using 2's complement and shifts result */
	int16_t signed_data[3];

	/* switch case for different shift operation depending on range (p.33) */
	switch (dev->range) {
	case range_2G:
		signed_data[0] = ((int16_t) raw_data[0]) >> 6;
		signed_data[1] = ((int16_t) raw_data[1]) >> 6;
		signed_data[2] = ((int16_t) raw_data[2]) >> 6;
		break;
	case range_4G:
		signed_data[0] = ((int16_t) raw_data[0]) >> 5;
		signed_data[1] = ((int16_t) raw_data[1]) >> 5;
		signed_data[2] = ((int16_t) raw_data[2]) >> 5;
		break;
	case range_8G:
		signed_data[0] = ((int16_t) raw_data[0]) >> 4;
		signed_data[1] = ((int16_t) raw_data[1]) >> 4;
		signed_data[2] = ((int16_t) raw_data[2]) >> 4;
		break;
	case range_16G:
...
HAL_StatusTypeDef ADXL345_ReadRawAcceleration(ADXL345 *dev, uint8_t *data_buf) {
	HAL_StatusTypeDef status;

	status = ADXL345_ReadRegisters(dev, ADXL345_REG_DATAX0, data_buf, 6);
...
HAL_StatusTypeDef ADXL345_WriteRegister(ADXL345 *dev, uint8_t reg, uint8_t *data) {
	...
	/* SPI START */
	HAL_GPIO_WritePin(GPIOx, GPIO_PIN_X, GPIO_PIN_RESET); // select chip line LOW

	status = HAL_SPI_Transmit(dev->hspi, spi_buf, WRITE_SINGLE_BYTE_SIZE, HAL_MAX_DELAY);

	HAL_GPIO_WritePin(GPIOx, GPIO_PIN_X, GPIO_PIN_SET); // select chip line HIGH
	/* SPI END */
