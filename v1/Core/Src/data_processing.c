...
void process_data_task(void *pvParameters) {
	...
	for (;;) {
		...
		if (*(data_processing->serial_raw_data_buffer_ready_flag) == 0) {
...
void write_serial_raw_data_buf(data_processing_t* data_processing, uint8_t* acc_data, uint8_t* gyro_data) {
	for (int i = 0; i<6;i++) {
		data_processing->serial_acc_raw_data_buf[i] = acc_data[i];
		data_processing->serial_gyro_raw_data_buf[i] = gyro_data[i];
	}
}

void read_raw_data_buf(data_processing_t* data_processing, uint8_t* acc_data, uint8_t* gyro_data) {
	for (int i = 0; i<6;i++) {
		acc_data[i] = data_processing->acc_raw_data_buf[i];
		gyro_data[i] = data_processing->gyro_raw_data_buf[i];
	}
}
